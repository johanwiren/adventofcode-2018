...
(defn p2-solution-list-and-mult-impl []
  (let [changes (map #(Integer/parseInt %)
                     (str/split input #" "))]
    (reduce (fn [[fs product] change]
              (let [f (+ (first fs) change)]
                (if (and
                     (zero? (mod product f))
                     (some #{f} fs))
...
(def p2-solution (delay (p2-solution-list-and-bloom-impl)))
